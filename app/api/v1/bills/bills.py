...
async def get_bill_by_uuid(
    bill_uuid: UUID, session: AsyncSession = Depends(get_async_session)
) -> IBillResponse:
    # TODO: AUTH HERE AHH
...
async def get_multi_bills(
    skip: Annotated[Union[int, None], Query] = 0,
    ...
    # TODO: AUTH AHH
    bills = await BillService.get_bills_for_user_multi_ordered(
        skip=skip,
        limit=limit,
        order_by=order_by,
        order=order,
...
@bills_router.post("")
async def post_bill(
    user_data: IBillCreate, session: AsyncSession = Depends(get_async_session)
) -> IBillRead:
    try:
    ...
    bill_new = await crud.bill.get_with_uuid_columns(
        uuid=bill.uuid, session=session, columns=["items"], read_interface=IBillRead
    )
    return bill_new
